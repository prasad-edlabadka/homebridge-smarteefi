...
export class SwitchAccessory {
  constructor(
    ...
    this.apiHelper = SmarteefiAPIHelper.Instance(
      new Config(
        platform.config.userid,
  ...
  async setOn(value: CharacteristicValue) {
    ...
    let statusmap =
      this.deviceStatus.getStatusMap(this.accessory.context.device.id)
        ?.statusmap || 0;
      ...
      (body) => {
        ...
        if (body.result != 'success') {
          this.platform.log.error(
            `Failed to change device status due to error ${body.msg}`,
          );
        } else {
          this.platform.log.info(
            `${this.accessory.displayName} is now ${
              (value as number) == 0 ? 'Off' : 'On'
            }`,
          );
  ...
  async getOn(): Promise<CharacteristicValue> {
    ...
    this.platform.log.debug(
      'Triggered GET On',
      statusmap,
      this.accessory.context.device.sequence,
      Number(statusmap & this.accessory.context.device.sequence),
    );
    if (statusmap == -1) {
    ...
    statusmap &= this.accessory.context.device.sequence;
    if (statusmap == 0) {
