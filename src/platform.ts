 ...
 */
export default class SmarteefiPlatform implements DynamicPlatformPlugin {
  ...
  discoverDevices() {
    ...
    const smarteefi: SmarteefiDiscovery = new SmarteefiDiscovery(
      this.log,
      this.api,
    );
        ...
        if (device) {
          ...
          if (existingAccessory) {
            ...
            if (Accessory) {
              ...
              this.log.warn(
                `Removing unsupported accessory '${existingAccessory.displayName}'...`,
              );
            }
  ...
  refreshStatus(_this, onetime = false) {
    ...
    const apiHelper = SmarteefiAPIHelper.Instance(
      new Config(
        _this.config.userid,
    ...
    for (let x = 0; x < totalDevices; x++) {
      ...
      apiHelper.getSwitchStatus(deviceId, 255, function (body) {
        ...
        if (body.result == 'error') {
          _this.log.error(
            `Unable to get status for deviceId ${deviceId}. Reason: ${_this.getReason(
              body.major_ecode,
            )}`,
          );
          _this.deviceStatus.setStatusMap(deviceId, -1, -1);
        } else {
          _this.deviceStatus.setStatusMap(
            deviceId,
            body.switchmap,
            body.statusmap,
          );
          ...
          for (let i = 0; i < totalSwitches; i++) {
            ...
            if (acc.context.device.id == deviceId) {
              ...
              const status = _this.decodeStatus(
                acc.context.device.sequence,
                acc.context.device.id,
              );
              svc?.updateCharacteristic(_this.Characteristic.On, status);
              _this.log.debug(
                `Status updated for '${acc.displayName}' to '${
                  status == 0 ? 'Off' : 'On'
                }'`,
              );
            }
